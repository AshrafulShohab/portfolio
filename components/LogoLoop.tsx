...
interface NodeLogo extends BaseLogo {
  node: React.ReactNode;
}
interface ImageLogo extends BaseLogo {
  src: string;
  srcSet?: string;
  sizes?: string;
  width?: number;
...
export const LogoLoopComponent: React.FC<LogoLoopProps> = ({
    logos,
    speed = 120,
    direction = 'left',
    width = '100%',
    ...
    useAnimationLoop(trackRef, targetVelocity, seqWidth, isItemHovered, pauseOnHover);
    ...
    const handleItemMouseEnter = useCallback((title: string | undefined) => {
      if (pauseOnHover) setIsItemHovered(true);
      onItemHover?.(title || null);
    }, [pauseOnHover, onItemHover]);

    const handleItemMouseLeave = useCallback(() => {
        if (pauseOnHover) setIsItemHovered(false);
        onItemHover?.(null);
    }, [pauseOnHover, onItemHover]);
    ...
    const renderLogoItem = useCallback((item: Logo, key: string) => {
      ...
      const itemAriaLabel = isNodeItem ? ((item as NodeLogo).ariaLabel ?? item.title) : ((item as ImageLogo).alt ?? item.title);
